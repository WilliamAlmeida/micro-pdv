<table class="table-auto text-xs w-full">
    ...
    <tbody>
        ...
        <tr class="h-[35px] border-y-2 border-gray-800 {{ $convenio_selecionado && $convenio_selecionado->id == $dados->id ?
        ...
        'bg-gray-300 hover:bg-gray-200 dark:border-gray-700 dark:bg-transparent dark:hover:bg-gray-700'
            ...
            <td class="text-end p-1">
                @if(!$convenio_selecionado || $convenio_selecionado && $convenio_selecionado->id != $dados->id)
                    <x-button primary xs icon="eye" wire:click="visualizar_convenio({{ $dados->id }})" class="invisible group-hover/item:visible" />
                @endif
            </td>
        </tr>
        @empty
