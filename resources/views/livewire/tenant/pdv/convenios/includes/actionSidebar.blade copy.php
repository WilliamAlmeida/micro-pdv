<div class="flex flex-col gap-y-2 h-full dark:text-white">
    ...
    <div class="sm:text-center mx-2 sm:mx-0">
        ...
        <div class="inline-block sm:hidden float-right">
            @if($convenio_selecionado)
                <x-button href="{{ route('tenant.pdv.convenios', tenant()) }}" label="Voltar" wire:navigate primary sm class="w-full" />
            @else
    ...
    <div class="flex flex-wrap justify-center sm:justify-normal sm:flex-nowrap sm:flex-col flex-grow overflow-x-auto">
        @include('livewire.pdv.convenios.includes.gridSaleTable')
    </div>
